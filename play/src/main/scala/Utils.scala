...
///** Used to merge the streams from StreamDemux,
//  * So there must be no more than one stream to be valid at the sanme time.
//  */
//class MergeDemuxStreams[T <: Data](dataType: HardType[T], val portCount: Int)
//    extends Component {
//  val io = new Bundle {
...
//  }
//
//  val validOH = io.inputs.map(_.valid).asBits()
//  assert(
//    assertion = CountOne(validOH) <= 1,
//    message =
//      L"merge streams having more than one valid, streams to merge should be demux-ed from the same original one",
//    severity = ERROR
//  )
//  for ((inputStream, idx) <- io.inputs.zipWithIndex) {
//    inputStream.ready := validOH(idx)
//  }
//  io.output << io.inputs.oneHotAccess(validOH)
//}
//
//object MergeDemuxStreams {
////  def apply[T <: Data](inputs: Stream[T]*): Stream[T] = {
////    MergeDemuxStreams(Vec(inputs.toIterable))
////  }
//
//  def apply[T <: Data](inputs: Seq[Stream[T]]): Stream[T] = {
//    val vec = Vec(inputs)
//    MergeDemuxStreams(vec)
//  }
//
//  def apply[T <: Data](inputs: Vec[Stream[T]]): Stream[T] = {
//    val merger = new MergeDemuxStreams(inputs(0).payload, inputs.length)
//    for ((a, b) <- merger.io.inputs.zip(inputs)) {
//      a << b
//    }
//    // merger.io.inputs.zip(inputs) //).map(case (x, y) => x << y)
//    merger.io.output
//  }
//}
...
class FragmentStreamJoinStream[T1 <: Data, T2 <: Data](
  ...
  io.outputJoinStream << io.inputFragmentStream
    ...
    .translateWith {
      val rslt = cloneOf(io.outputJoinStream.payload)
...
//========== DMA responses related utilities ==========

//object genMultiPktFromDmaReadResp {
//  def getCurOpCode(dmaRespOpCodeStart: Bits,
//                   isFirstPkt: Bool,
//                   isLastPkt: Bool,
//                   hasImmDt: Bool,
//                   hasInv: Bool): Bits =
//    new Composite(dmaRespOpCodeStart) {
//      val rslt = Bits(OPCODE_WIDTH bits)
//      val isSendReq = OpCode.isSendReqPkt(dmaRespOpCodeStart)
//      val isWriteReq = OpCode.isWriteReqPkt(dmaRespOpCodeStart)
//      val isReadResp = OpCode.isReadRespPkt(dmaRespOpCodeStart)
//      val isOnlyPkt = OpCode.isSendOnlyReqPkt(dmaRespOpCodeStart) || OpCode
//        .isWriteOnlyReqPkt(dmaRespOpCodeStart) || OpCode.isOnlyReadRespPkt(
//        dmaRespOpCodeStart
//      )
//      val isLastPkt = OpCode.isSendLastReqPkt(dmaRespOpCodeStart) || OpCode
//        .isWriteLastReqPkt(dmaRespOpCodeStart) || OpCode.isLastReadRespPkt(
//        dmaRespOpCodeStart
//      )
//      assert(
//        assertion = isSendReq || isWriteReq || isReadResp,
//        message =
//          L"multi-packet opcode=${dmaRespOpCodeStart} can only be send/write req and read resp",
//        severity = FAILURE,
//      )
//      when(isFirstPkt && isLastPkt) {
//        assert(
//          assertion = OpCode.isLastOrOnlyReqPkt(dmaRespOpCodeStart) || OpCode
//            .isLastOrOnlyReadRespPkt(dmaRespOpCodeStart),
//          message =
//            L"""when isFirstPkt=${isFirstPkt} and isLastPkt=${isLastPkt},
//             opcode=${dmaRespOpCodeStart} should be last or only send/write request or read response""",
//          severity = FAILURE
//        )
//      }
//
//      rslt := dmaRespOpCodeStart
//
//      when(isSendReq) {}
//      when(isWriteReq) {
//        when(!isFirstPkt && isLastPkt) {
...
//          }
//        } elsewhen (!isFirstPkt && !isLastPkt) {
//          rslt := OpCode.RDMA_WRITE_MIDDLE.id
//        } elsewhen (isFirstPkt && isLastPkt) {
//          assert(
//            assertion = OpCode.isOnlyReadRespPkt(dmaRespOpCodeStart),
//            message =
//              L"""when isFirstPkt=${isFirstPkt} and isLastPkt=${isLastPkt},
//             opcode=${dmaRespOpCodeStart} should be read response only""",
//            severity = FAILURE
//          )
//          when(hasImmDt) {
...
//    }.rslt
//
//  // TODO: check whether solicited bit is set for last packet or all packets?
//  def apply(dmaReadResp: Stream[Fragment[DmaReadResp]],
//            qpAttr: QpAttrData,
//            flush: Bool,
//            signaled: Bool,
//            solicited: Bool,
//            busWidth: BusWidth): Stream[DataAndMty] =
//    new Composite(dmaReadResp) {
////    val readRespHeaderBitLen = widthOf(BTH()) + widthOf(AETH())
////    val readRespHeaderLenBytes = readRespHeaderBitLen / BYTE_WIDTH
////    val bthHeaderLenBytes = widthOf(BTH()) / BYTE_WIDTH
//      val busWidthBytes = busWidth.id / BYTE_WIDTH
//
//      val inputValid = dmaReadResp.valid
//      val inputDmaDataFrag = dmaReadResp.fragment
//      val isLastDmaDataFrag = dmaReadResp.isLast
//      val isFirstDmaDataFrag = dmaReadResp.isFirst
//
//      val numReadRespPkt =
//        divideByPmtuUp(inputDmaDataFrag.totalLenBytes, qpAttr.pmtu)
//      val lastOrOnlyReadRespPktLenBytes =
//        moduloByPmtu(inputDmaDataFrag.totalLenBytes, qpAttr.pmtu)
//
//      val pmtuFragNum = pmtuPktLenBytes(qpAttr.pmtu) >> log2Up(
//        busWidth.id / BYTE_WIDTH
//      )
//      val segmentStream = StreamSegment(
//        dmaReadResp.throwWhen(flush).translateWith {
//          val rslt = Fragment(DataAndMty(busWidth.id))
//          rslt.data := dmaReadResp.data
//          rslt.mty := dmaReadResp.mty
//          rslt.last := dmaReadResp.last
//          rslt
//        },
//        fragmentNum = pmtuFragNum.resize(PMTU_FRAG_NUM_WIDTH)
//      )
//
//      // Count the number of packet processed
//      val pktCnt = Counter(
//        bitCount = (RDMA_MAX_LEN_WIDTH / busWidth.id) bits,
//        inc = segmentStream.fire && segmentStream.isLast
//      )
//
//      val curPsn = dmaReadResp.psnStart + pktCnt.value
...
//        } elsewhen (pktCnt.value === numReadRespPkt - 1) {
//          opcode := OpCode.RDMA_READ_RESPONSE_LAST.id
//          padcount := (PADCOUNT_FULL - lastOrOnlyReadRespPktLenBytes(
//            0,
//            PADCOUNT_WIDTH bits
