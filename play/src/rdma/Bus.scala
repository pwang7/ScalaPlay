...
//  val rnrExc = Bool()
  ...
  def hasFatalErr(): Bool = {
    when(pulse) {
      assert(
        ...
        message =
          L"SqErrNotifier.pulse=${pulse}, but errType=${errType} shows no error".toSeq,
        severity = FAILURE
      )
    }
    pulse
  ...
  def ||(that: SqErrNotifier): SqErrNotifier = {
    assert(
      ...
      message =
        L"cannot merge two SqErrNotifier both have fatal error, this.pulse=${this.pulse}, this.errType=${this.errType}, that.pulse=${that.pulse}, that.errType=${that.errType}".toSeq,
      severity = FAILURE
    )
    ...
    new Composite(this) {
      ...
      switch(rnrTimeOut) {
        ...
        default {
          report(
            message =
              L"${REPORT_TIME} time: invalid rnrTimeOut=${rnrTimeOut}, should between 0 and 31".toSeq,
            severity = FAILURE
          )
          result := 0
        }
  ...
  def deMuxRespByInitiator(
    ...
    switch(resp.initiator) {
      ...
      default {
        report(
          message =
            L"${REPORT_TIME} time: invalid DMA initiator=${resp.initiator}, should be RQ_RD, RQ_DUP, RQ_ATOMIC_RD, SQ_RD, SQ_DUP".toSeq,
          severity = FAILURE
        )
        txSel := otherIdx
      }
    ...
    new Composite(this) {
      switch(retryReason) {
        ...
        default {
          report(
            message =
              L"input retryReason=${retryReason} should be valid reason".toSeq,
            severity = FAILURE
          )
        }
      }
...
//    val status = Bits(WC_STATUS_WIDTH bits)
  ...
  def setOpCodeFromRqReqOpCode(reqOpCode: Bits): this.type = {
    when(OpCode.isSendReqPkt(reqOpCode)) {
      ...
      report(
        message =
          L"${REPORT_TIME} time: unmatched WC opcode at RQ site for request opcode=${reqOpCode}".toSeq,
        severity = FAILURE
      )
