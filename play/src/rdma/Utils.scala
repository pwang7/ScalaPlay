    ...
    new Composite(signal) {
      ...
      when(validReg) {
        assert(
          ...
          message =
            L"${REPORT_TIME} time: previous signal edge triggered stream data hasn't fired yet, validReg=${validReg}, signalRiseEdge=${signalRiseEdge}".toSeq,
          severity = FAILURE
        )
      }
...
//    respStream: Stream[Tresp],
//    waitQueueDepth: Int,
//    buildQuery: Tin => Tquery,
//    queryCond: Stream[Fragment[Tin]] => Bool,
//    expectResp: Stream[Fragment[Tin]] => Bool,
//    joinRespCond: Stream[Fragment[Tin]] => Bool
//  ): Stream[Fragment[TupleBundle2[Tin, Tresp]]] = {
//    val result = new FragmentStreamForkQueryJoinResp(
//      inputFragStream.fragmentType,
//      queryStream.payloadType,
//      respStream.payloadType,
//      waitQueueDepth,
//      buildQuery = buildQuery,
//      queryCond = queryCond,
//      expectResp = expectResp,
//      joinRespCond = joinRespCond
//    )
//    result.io.input << inputFragStream
//    queryStream << result.io.query
//    result.io.resp << respStream
//    result.io.output
//  }
//}
//
//class FragmentStreamForkQueryJoinResp[Tin <: Data,
//                                      Tquery <: Data,
//                                      Tresp <: Data](
//  fragmentType: HardType[Tin],
//  queryType: HardType[Tquery],
//  respType: HardType[Tresp],
//  waitQueueDepth: Int,
//  buildQuery: Tin => Tquery,
//  queryCond: Stream[Fragment[Tin]] => Bool,
//  expectResp: Stream[Fragment[Tin]] => Bool,
...
//) extends Component {
//  val io = new Bundle {
//    val input = slave(Stream(Fragment(fragmentType())))
//    val query = master(Stream(queryType()))
//    val resp = slave(Stream(respType()))
//    val output = master(Stream(Fragment(TupleBundle2(fragmentType, respType))))
//  }
//
//  val (input4QueryStream, originalInputStream) =
//    StreamConditionalFork2(io.input, forkCond = queryCond(io.input))
//  val originalInputQueue = originalInputStream.queueLowLatency(waitQueueDepth)
//  io.query <-/< input4QueryStream ~~ (
//    payloadData => buildQuery(payloadData.fragment)
//  )
//
//  // When not expect query response, join with StreamSource()
//  val stream4Join = StreamMux(
//    select = expectResp(originalInputQueue).asUInt,
//    inputs =
//      Vec(StreamSource().translateWith(respType().assignDontCare()), io.resp)
//  )
//  val joinStream =
//    FragmentStreamConditionalJoinStream(
//      originalInputQueue,
  ...
  def apply[T <: Data](input: Stream[T], select: Bits): Vec[Stream[T]] = {
    ...
    when(input.valid) {
      assert(
        ...
        message =
          L"${REPORT_TIME} time: select=${select} should be one hot".toSeq,
        severity = FAILURE
      )
    }
...
class StreamOneHotDeMux[T <: Data](dataType: HardType[T], portCount: Int)
  ...
  when(io.input.valid) {
    assert(
      ...
      message =
        L"${REPORT_TIME} time: io.select=${io.select} is not one hot".toSeq,
      severity = FAILURE
    )
  }
  ...
  def apply[T <: Data](input: Stream[T], conditions: Bool*): Vec[Stream[T]] = {
    ...
    when(input.valid) {
      assert(
        ...
        message =
          L"de-mux conditions=${oneHot} should have exact one true condition when input stream is valid".toSeq,
        severity = FAILURE
      )
    }
    ...
    new Composite(retryWorkReq) {
      ...
      when(retryWorkReqValid && !retryFromBeginning) {
        ...
        assert(
          ...
          message =
            L"${REPORT_TIME} time: io.qpAttr.retryStartPsn=${qpAttr.retryStartPsn} should < retryWorkReq.psnStart=${retryWorkReq.psnStart} + retryWorkReq.pktNum=${retryWorkReq.pktNum} = ${retryWorkReq.psnStart + retryWorkReq.pktNum} in PSN order".toSeq,
          severity = FAILURE
        )
        ...
        assert(
          ...
          message =
            L"${REPORT_TIME} time: psnDiff=${psnDiff} should < pktNum=${pktNum}, qpAttr.retryStartPsn=${qpAttr.retryStartPsn}, retryWorkReq.psnStart=${retryWorkReq.psnStart}, qpAttr.npsn=${qpAttr.npsn}, retryWorkReq.workReq.opcode=${retryWorkReq.workReq.opcode}".toSeq,
          severity = FAILURE
        )
      }
